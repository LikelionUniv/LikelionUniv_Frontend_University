...
const Header = () => {
    ...
    const profileImageUrl = data[0].post.hasAuthorProfileImageUrl ? data[0].post.authorProfileImageUrl : profileImage;
    ...
    return (
        <>
            ...
            <D.User>
                <div className='left'>
                    ...
                    <div>
                        <div className='userBox'>
                            ...
                            {!isMyPost && (
                              <div className={`followBtn ${isFollowed ? 'followed' : ''}`} onClick={toggleFollow}>
                                 {isFollowed ? <span>팔로잉</span> : <span>팔로우</span>}
                            </div>
                            )}
                        </div>
                        <div className='userInfo'>
                            <p>{data[0].post.authorOrdinal}기</p><D.Dot />
                            <p>{data[0].post.universityName}</p><D.Dot />
                            <p>2023. 7. 18</p>
                        </div>
                    </div>
                </div>
                <div className='right'>
                    <div className='icons'>
                        <HeartIcon />{data[0].post.likeCount}
                    </div>
                    <div className='icons'>
                        <CommentIcon />{data[0].post.commentCount}
                    </div>
                    {isMyPost && (
                    <div className='btns'>
                        <p className='delete'>삭제하기</p>
                        <p className='modify' onClick={goModify}>수정하기</p>
                    </div>)}
                </div>
